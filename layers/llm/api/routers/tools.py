...
async def sysml(option_uuid: str):
    try:
        ...
        # Hardcoded list of document UUIDs to retrieve
        document_uuids = [
            # Add your document UUIDs here
            "a1f282d3-bb1e-4bac-9503-e4efc88567ad"
        ]
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response = await client.get(f"http://knowledge-api:10000/options/{option_uuid}")
...
Based on this context, please analyze the following option: {option["name"]}"""
            ...
            async with httpx.AsyncClient(timeout=120.0) as llm_client:
                llm_response = await llm_client.post(api_endpoint, json=payload, headers=headers)
